...
void HandleTCPClient(int clntSocket, int n, int* book_info, const char sem_name[], int number)
{
    setbuf(stdout, NULL);
    ...
    while(1) {
        ...
        printf("Reader %d wants to take the following books for %d days:", number, duration);
        ...
        while (1) {
            ...
            for (int i = 0; i < count; ++i) {
                ...
                if (books[i] != -1 && !book_info[books[i]]) {     // Книга свободна.
                    ...
                    printf("Reader %d takes the book number %d for %d days\n", number, books[i], duration);
