...
void message_cpy(sem_t* sem, char* message, char* source) {
    if (sem_wait(sem) == -1) {
        ...
        for (;;) /* Run forever */
        {
            ...
            else if (processID == 0)  /* If this is the child process */
            {
                ...
                if (code == 1) {    // Подключился читатель.
                    ...
                    printf(buffer);
                    message_cpy(sem_msg, message, buffer);

                    HandleTCPClient(clntSock, n, books_info, sem_name, *counter, message);
                }
                else {
                    ...
                    HandleTCPMonitor(clntSock, message);
                }
