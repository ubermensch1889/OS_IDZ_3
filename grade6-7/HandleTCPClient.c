...
void message_copy(sem_t* sem, char* message, char* source) {
    if (sem_wait(sem) == -1) {
...
void HandleTCPClient(int clntSocket, int n, int* book_info, const char sem_name[], int number, char* message)
{
    setbuf(stdout, NULL);
    ...
    while(1) {
        ...
        sprintf(info, "%s\n", info);
        printf(info);
        message_copy(sem_msg, message, info);
